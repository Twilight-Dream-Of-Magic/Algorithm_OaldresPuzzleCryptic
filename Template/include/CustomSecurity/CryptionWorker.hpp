...
namespace Cryptograph
{
	namespace OaldresPuzzle_Cryptic::Version1
	{
		...
		*/
		...
		public:
			std::uint64_t operator()(std::uint64_t round)
			{
				y = (x ^ std::rotl(state, 32)) ^ std::rotl(state, 19);
				if(x == 0)
					x = ROUND_CONSTANT[round % ROUND_CONSTANT.size()];
				else
					x += std::rotl(x, 7) ^ ROUND_CONSTANT[round % ROUND_CONSTANT.size()];
		...
		public:
			std::uint64_t EncryptionCoreFunction(const std::uint64_t data, const std::uint64_t key, const std::uint64_t round)
			{
				...
				switch ( choise_function )
				{
					...
					case 2:
					{
						result = std::rotl( data, bit_rotation_amount );
						break;
					}

					case 3:
					{
						result = std::rotr( data, bit_rotation_amount );
						break;
					}
					default:
						break;
				...
				result += (std::rotr(key, 3) ^ std::rotr(subkey, 11));
			...
			std::uint64_t DecryptionCoreFunction(const std::uint64_t data, const std::uint64_t key, const std::uint64_t round)
			{
				...
				result = data - (std::rotr(key, 3) ^ std::rotr(subkey, 11));
				...
				switch (choise_function)
				{
					...
					case 2:
					{
						result = std::rotr(result, bit_rotation_amount);
						break;
					}
					case 3:
					{
						result = std::rotl(result, bit_rotation_amount);
						break;
					}
					default:
						break;
