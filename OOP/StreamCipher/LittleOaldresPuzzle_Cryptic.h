...
namespace TwilightDreamOfMagical::CustomSecurity
{
	...
	namespace SED::StreamCipher
	{
		...
		public:
			...
			std::uint64_t SingleRoundEncryption(const std::uint64_t data, const std::uint64_t key, const std::uint64_t round)
			{
				std::uint64_t result = EncryptionCoreFunction(data, key, round);
				prng.Seed(seed);
				return result;
			}

			std::uint64_t SingleRoundDecryption(const std::uint64_t data, const std::uint64_t key, const std::uint64_t round)
			{
				std::uint64_t result = DecryptionCoreFunction(data, key, round);
				prng.Seed(seed);
				return result;
			}
