...
namespace TwilightDreamOfMagical::CustomSecurity
{
	namespace CSPRNG
	{
		...
		*/
		...
		XorConstantRotation::result_type XorConstantRotation::StateIteration(std::size_t round)
		{
			y = (x ^ BaseOperation::rotate_left(state, 32)) ^ BaseOperation::rotate_left(state, 19);
			if(x == 0)
				x = ROUND_CONSTANT[round % ROUND_CONSTANT.size()];
			else
				x += BaseOperation::rotate_left(x, 7) ^ ROUND_CONSTANT[round % ROUND_CONSTANT.size()];
