#include "XorConstantRotation_balanced_256_whitebox.h"
#include <numeric>

namespace TwilightDreamOfMagical::CustomSecurity::CSPRNG
{
	// NOTE:
	// - First 4 constants are "manually mixed anchors" (your design).
	...
	const std::array<std::uint64_t, 300> XorConstantRotation::ROUND_CONSTANT =
	{
	...
	XorConstantRotation::XorConstantRotation()
		: w(ROUND_CONSTANT[0]), x(0), y(0), z(0)
	{
		StateInitialize();
	}
	...
	static constexpr std::size_t ROUND_CONSTANT_SIZE = ROUND_CONSTANT.size();
	...
	void XorConstantRotation::StateInitialize()
	{
		...
		// split
		const std::uint32_t left_seed  = static_cast<std::uint32_t>((random >> 32) ^ ROUND_CONSTANT[ROUND_CONSTANT_SIZE - 1]);
		const std::uint32_t right_seed = static_cast<std::uint32_t>((random & 0xFFFFFFFFU) ^ ROUND_CONSTANT[ROUND_CONSTANT_SIZE - 2]);
	...
	XorConstantRotation::result_type XorConstantRotation::StateIteration(std::size_t number_once)
	{
		...
		const std::uint64_t RC0 = ROUND_CONSTANT[( n ) % ROUND_CONSTANT_SIZE];
		const std::uint64_t RC1 = ROUND_CONSTANT[( counter ) % ROUND_CONSTANT_SIZE];
		const std::uint64_t RC2 = ROUND_CONSTANT[( n + counter ) % ROUND_CONSTANT_SIZE];
		const std::uint64_t RC3 = ROUND_CONSTANT[( (n ^ std::rotl(n ^ counter,3)) ) % ROUND_CONSTANT_SIZE];
